...
var FingerPrint = (function() {
    ...
    // in case we need specific hacks for better cross-browsers support, we wrap these ones

    function get_user_agent() {
        return navigator.userAgent || '';
    }

    // We use the following algorithm to find out the timestamps of the days where the
    // TZ changed during the time frame from 1st Jan. 1970 to 1st Jan 1990.
    // This is a constant that is the same for each country in the same TZ.
    // We don't use getTimezoneOffset() because it would lead to a FP reset 
    // whenever the time is changed.
    function get_timezone() {
        var d = new Date(), current_time = d.getTime(), one_day = 86400000, tz_changes = '', oldTime  = d.getTimezoneOffset();

        for (var i = 0; i < 630720000000; i += one_day) {
            d.setTime(i);
            newTime = d.getTimezoneOffset();
            if (newTime != oldTime) {
                tz_changes += (newTime + '@' + Math.round(i / 1000) + ';');
                oldTime = newTime;
            }
        }
        return tz_changes;
    ...
    function get_plugins() {
        ...
        for (i=0; i<navigator.plugins.length; i++) {
            plugins_str += navigator.plugins[i].name || '';
        }
    ...
    return { 
       ...
       'get': function() { return MD5.base64(seed); }
    };
})();
